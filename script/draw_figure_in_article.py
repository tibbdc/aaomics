...
def Comparison_of_protein_MW_by_species(strain_list1,strain_list2,fig_file,analysis_result):
    ...
    pngname='%s/protein_MW_boxplot_total.png'%fig_file
...
'''2.2 Amino acid mass ratio of each protein'''
...
'''2.3 Mass ratio of total amino acids'''
...
'''2.5 Total amino acid mass of protein'''
def Proteome_Total_amino_acid_mass_of_protein(fig_file,strain,analysis_result,topnum,use_prot,protein_mass_norm_stat_std):
    ...
    if stats.levene(data1,data2).pvalue>0.5:
        print('Standard independent 2 sample test')
        mw_stat.loc['exp2amino','p_value']=stats.ttest_ind(data1,data2).pvalue
    else:
...
'''3.1 Amino acid mass ratio of each protein'''
...
'''3.2 Total amino acid mass of protein'''
...
'''4.3 AAs mass distribution total protein'''
def AAs_mass_distribution_total_protein(fig_file,strain_list,strainName,analysis_result):
    Species_Number = len(strain_list)
    createVar = locals()
...
def Comparison_of_other_species(fig_file,strain_list,strainName,analysis_result):
    ...
    AAs_mass_distribution_total_protein(fig_file,strain_list,strainName,analysis_result)
