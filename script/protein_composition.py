...
def protein_expression_mass_norm(proteome_file,gene_uniprot_select_outfile,protein_expression_mass_norm_outfile):
    ...
    for index, row in my_pro_exp_T.iterrows():
        ...
        #break
    json_write(protein_expression_mass_norm_outfile, protein_expression_mass_norm)  # +'_exp_onecell.json'
...
def draw_amino_mass_in_total_protein(amino_composition_norm_onecell_df,pngname_boxplot,pngname_violinplot):
    ...
    plt.ylabel("Mass ratio (g/g total protein)",fontsize=22)
    plt.xlabel("Amino acid", fontsize=22)
    plt.savefig(pngname_boxplot,dpi =300,bbox_inches='tight')
    
    # plt.figure(figsize=(25, 10)) 
    # plt.tick_params(labelsize=20)
    # sns.violinplot(data=twoamino)
    # plt.ylabel("Mass ratio (g/g total protein)",fontsize=22)
    # plt.xlabel("Amino acid", fontsize=22)
    # plt.savefig(pngname_violinplot,dpi =300,bbox_inches='tight')
...
def draw_Total_MassAndExpression(data,columns_name,legend,pngname):
    num = 0
    width = 0.15
    ...
    for column in data.columns:
        ...
        # 对于每个值进行归一化
        data[column] = data[column] / column_sum

    for i in range(0, len(columns_name)):
        num = i*width
        # 计算标准误差
        std_err = np.std(data[columns_name[i]], ddof=1) / np.sqrt(len(data[columns_name[i]]))
        # print('std_err:',std_err)
        plt.bar(ind+num,data[columns_name[i]],width,label = legend[i])
        plt.errorbar(ind+num,data[columns_name[i]], yerr=std_err, fmt='.', color='black', capsize=5)
    
    plt.ylim(bottom=0)
    plt.xticks(np.arange(20) + ((len(columns_name)-1)/2)*width, list(data.index),rotation=0)

    plt.ylabel("Mass ratio (g/g) (Normalized)",fontsize=20)
